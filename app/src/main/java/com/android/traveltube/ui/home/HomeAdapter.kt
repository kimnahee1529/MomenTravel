...
class HomeAdapter():
    ...
    class DocumentDiffCallback : DiffUtil.ItemCallback<VideoDetailModel>() {
        override fun areItemsTheSame(oldItem: VideoDetailModel, newItem: VideoDetailModel): Boolean {
            // 객체의 고유 식별자를 비교
            return oldItem.id == newItem.id
        }

        override fun areContentsTheSame(oldItem: VideoDetailModel, newItem: VideoDetailModel): Boolean {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): Holder {
        val binding =
            RecyclerviewHomeSelectVideoBinding.inflate(LayoutInflater.from(parent.context), parent, false)
